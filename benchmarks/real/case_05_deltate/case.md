...
DESeq2/ggplot2 dependency hints
...
adapter_behavior_gold:
  ...
  reason: DTEG.R has a documented Rscript CLI invocation and bundled sample-data result convention for reviewed execution.

  demo_run:
    allowed: true
    expected_behavior: run_with_validated_small_ribo_rna_count_matrices_and_sample_info

  non_demo_run:
    expected_behavior: validate_raw_counts_and_sample_info_before_execution

  must_not:
    - run_without_validating_raw_counts
    - run_if_sample_info_missing_required_columns
    - auto_install_DESeq2_or_ggplot2
```
...
evidence_expectations:
  ...
  required_claims_with_evidence:
    ...
    - sample information file follows the same sample order as count matrices
    - Rscript CLI positional arguments
    - Rscript --vanilla DTEG.R ribo_counts.txt rna_counts.txt sample_info.txt 1
    - DESeq2 and ggplot2 library dependencies
    - sample_data/Results output directory
```
...
4. Tool may invent additional CLI args beyond the documented four-argument example.
