...
from paper2skill.evaluation.load_gold import evaluation_result, finish_result, flatten_strings
from paper2skill.evaluation.schemas import EXECUTABLE_ADAPTER_STATUSES, VALID_ADAPTER_STATUSES
...
def validate_skill_package(skill_dir: str | Path, gold: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    gold = gold or {}
    result = evaluation_result("skill_package")
...
def adapter_lifecycle_report(root: Path) -> dict[str, Any]:
    ...
    statuses = [str(value).lower() for value in flatten_strings({"adapter_spec": spec, "adapter_review": review}) if str(value).lower() in VALID_ADAPTER_STATUSES]
