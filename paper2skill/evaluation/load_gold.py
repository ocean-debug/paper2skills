...
def load_generated_bundle(generated: str | Path) -> dict[str, Any]:
    root = Path(generated)
    bundle: dict[str, Any] = {"generated_dir": str(root), "files": {}, "missing_files": [], "warnings": []}
    if not root.exists():
        bundle["warnings"].append(f"generated path does not exist: {root}")
    for key, filename in REFERENCE_FILES.items():
        path = root / filename
        if key == "environment_spec" and not path.exists():
            sibling_assets = root.parent / "assets" / filename
